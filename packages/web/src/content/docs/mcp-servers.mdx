...
If your project has a `.mcp.json` file at its root — the convention used by Claude Code, Claude Desktop, and Cursor — OpenCode automatically discovers it and translates its `mcpServers` into MCP servers, so you don't have to hand-translate a config you already have.
...
- `command`/`args`/`env`/`cwd` entries become `type: "local"` servers; `url`/`headers` entries (including `type: "http"`, `"streamable-http"`, and the deprecated `"sse"`) become `type: "remote"` servers.
- `type: "ws"` (WebSocket) servers aren't supported by OpenCode's remote MCP client and are skipped, with a warning logged naming the server.
- Bash-style `${VAR}` environment values are translated to OpenCode's own `{env:VAR}` syntax. The `${VAR:-default}` fallback form has no OpenCode equivalent — that environment key is dropped entirely (rather than baking in an empty string), with a warning logged naming the variable and the default that's being dropped. Export the variable directly if you rely on that default.
- **Explicit `mcp` entries in `opencode.json`/`opencode.jsonc` always take precedence over anything auto-discovered from `.mcp.json`.** If a server name exists in both, your explicit config wins in full — auto-discovery never overrides a server you've configured by hand.
