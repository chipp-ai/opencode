...
- **Status — phased rollout, each phase independently shipped and tested:**
  ...
  - ⬜ **Phase 2 — structured output.** Schema-forced tool-call output in the V2 runner (100% V1-only today).
  - ⬜ **Phase 3 — persistence.** Run journal/pause/resume, worktree isolation, cancellation/timeout wiring (the `workflow_run` schema and worktree mechanics from the community attempt are reusable near-verbatim).
...
- **Where:** [`dev` branch history](https://github.com/chipp-ai/opencode/commits/dev), commits `c76c57f`..`2a1fa60` (Phase 0+1).
